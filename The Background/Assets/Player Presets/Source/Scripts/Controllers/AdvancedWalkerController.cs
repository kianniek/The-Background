...
namespace CMF
{
    ...
    public class AdvancedWalkerController : Controller {
        ...
        //get from MovementController if climbing
        bool climbing;
        ...
        void Awake () {
            ...
            StoredRunningSpeed = runningSpeed;
        ...
        void Update()
        {
            climbing = movementController.climbing;
            if (!climbing)
            {
                HandleJumpKeyInput();
            }
        ...
        protected virtual Vector3 CalculateMovementVelocity()
        {
            ...
            if (characterInput.IsRunningKeyPressed())
            {
                _velocity *= Mathf.Lerp(movementSpeed, runningSpeed, 0.3f);
            }
            else if(characterInput.IsRunningKeyLetGo())
            {
                _velocity *= Mathf.Lerp(runningSpeed, movementSpeed, 0.3f);
            }
            else
            {
        ...
        protected virtual bool IsRunningKeyPressed()
        {
            //If no character input script is attached to this object, return;
            if (characterInput == null)
                return false;

            return characterInput.IsRunningKeyPressed();
        }
        ...
        void HandleMomentum()
        {
            ...
            //Moving in air can only be done with movement speed (not with running speed!)
            if(!IsGrounded())
            {
